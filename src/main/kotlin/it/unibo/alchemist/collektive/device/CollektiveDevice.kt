...
class CollektiveDevice<P>(
    ...
    private val retainMessagesFor: Time? = DoubleTime(5.0),
    ...
    override fun <T> set(name: String, value: T): T =
        value.also { node.setConcentration(SimpleMolecule(name), it) }

    fun <T> Int.set(name: String, value: T): T {
        val node = environment.getNodeByID(this)
        return value.also { node.setConcentration(SimpleMolecule(name), value) }
    }
}
